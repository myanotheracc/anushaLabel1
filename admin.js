...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdiaGhyaWZhd3V0bXRhY2F6aG51Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI0NTg0MDAsImV4cCI6MjA4ODAzNDQwMH0.DKkGoA2nT_-wJEj_ZO6d7eZwNKwoszXVUKQ00nLyuSsY';
const CLOUD_NAME = 'dfkuht22n'; 
const UPLOAD_PRESET = 'anusha_sarees'; 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 1. REAL LOGIN LOGIC (admin-login.html)
    // ==========================================
    ...
    if (loginBtn) {
        loginBtn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                msg.innerText = "Network timeout. Try deploying online or changing Wi-Fi.";
                msg.style.display = 'block';
                loginBtn.innerText = "Login";
            }
        });
    ...
    if (logoutBtn) {
        ...
        tabBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if(targetId === 'modify-product') {
                    loadModifyList();
                }
            });
        });

        // Cloudinary Upload Logic (With network error catch)
        let uploadedImages = [];
        ...
        window.openCloudinaryWidget = function() {
            if(window.cloudinary) {
                try {
                    ...
                    alert("Image uploader failed to open. Check your network or Ad-blocker.");
                }
            }
        }
        ...
        if (uploadBtn) {
            uploadBtn.addEventListener('click', openCloudinaryWidget, false);
        }

        window.renderThumbnails = function() {
            const container = document.getElementById('image-preview-container');
            container.innerHTML = `<div class="add-img-box" id="upload-widget-btn-rebind" style="width: 100px; height: 100px; border: 2px dashed #ccc; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 2rem; color: #ccc; cursor: pointer;">+</div>`;
            ...
            uploadedImages.forEach((url, index) => {
                ...
                div.innerHTML = `
                    ...
                    <button onclick="removeImg(${index})" style="position: absolute; top: -8px; right: -8px; background: #D81B60; color: white; width: 22px; height: 22px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 12px; font-weight: bold; border: none;">-</button>
                `;
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', async () => {
                ...
                if(!type || !name) return alert('Type and Name are required!');
                ...
                try {
                    ...
                    alert('Network timeout while saving to Supabase.');
                }
        ...
        window.loadModifyList = async function() {
            ...
            try {
                    ...
                    data.forEach(item => {
                        ...
                        container.innerHTML += `
                            <div style="display: flex; align-items: center; justify-content: space-between; padding: 15px; border: 1px solid #eee; border-radius: 8px; margin-bottom: 10px;">
                                ...
                                <div>
                                    <i class="fas fa-edit" style="color: #3498db; cursor: pointer; margin-right: 15px; font-size: 1.2rem;" onclick='editProduct(${JSON.stringify(item)})'></i>
                                    <i class="fas fa-trash" style="color: #e74c3c; cursor: pointer; font-size: 1.2rem;" onclick="deleteProduct('${item.id}')"></i>
                                </div>
                            </div>`;
                    });
                } else {
                ...
                container.innerHTML = `<p style="color:red">Network Timeout.</p>`;
            }
        }
        ...
        window.deleteProduct = async (id) => {
            if(confirm('Delete this product?')) {
                try {
                    ...
                    alert("Timeout deleting product.");
                }
            }
        }
            ...
            postIgBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if(error) alert(error.message);
                        ...
                        alert('Instagram Link Posted!');
                        document.getElementById('ig-link').value = '';
                        document.getElementById('ig-desc').value = '';
                        loadIGList();
                    }
                } catch(e) {
                    alert('Network Timeout posting Instagram link.');
                }
                    ...
                    data.forEach(item => {
                        container.innerHTML += `
                            <div style="display: flex; justify-content: space-between; padding: 10px; border-bottom: 1px solid #eee;">
                                ...
                                <i class="fas fa-trash" style="color:red; cursor:pointer;" onclick="deleteIG('${item.id}')"></i>
                            </div>`;
                    });
                }
            } catch(e) {}
